...
export const TimetablePanelMixin = {
    updateTimetable() {
        ...
        active.forEach(t => {
            ...
            html += `<div class="tt-row" onclick="MapSystem.focusTrain('${t.id}')">
                <div style="flex: 1;"><strong>${t.id}</strong> ➔ ${NODES[t.destFinal].name}</div>
                <div style="flex: 1; color:#aaa;">${loc}</div>
                <div style="width: 50px; text-align:right;">${delay}</div></div>`;
        });
        list.innerHTML = html;
    }
};
